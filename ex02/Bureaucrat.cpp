...
void Bureaucrat::signForm(AForm& form) {
	try {
		form.beSigned(*this);
		std::cout << "[SIGNATURE] " << _name << " signed " << form.getName() << std::endl;
	} catch (std::exception& e) {
		std::cout << "[DENIED] " << _name << " cannot sign " << form.getName() 
				  << " - " << e.what() << std::endl;
	}
}
...
void Bureaucrat::executeForm(AForm const & form) {
	try {
		form.execute(*this);
		std::cout << "[EXECUTION] " << _name << " executed " << form.getName() << std::endl;
	} catch (std::exception& e) {
		std::cout << "[BLOCKED] " << _name << " cannot execute " << form.getName() 
				  << " - " << e.what() << std::endl;
	}
}
...
std::ostream& operator<<(std::ostream& out, const Bureaucrat& bureaucrat) {
	out << "[Bureaucrat Info] Name: " << bureaucrat.getName() 
	    << " | Grade: " << bureaucrat.getGrade() << "/150";
	return out;
}
